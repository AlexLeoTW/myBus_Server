...
router.post('/reservation',
    ...
    (req, res) => {
        ...
        db.query(`SELECT COUNT(*) AS violationCount FROM \`violation_log\` WHERE \`UID\` = ${req.body.UID} AND \`timestamp\` BETWEEN timestamp(DATE_SUB(NOW(), INTERVAL 30 DAY)) AND NOW()`)
            .then( (result) => {
